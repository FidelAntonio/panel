...
class Archivos extends CI_Model
{
	...
	function subirArchivo($nombreArchivo, $ruta = "assets/archivos") {
		...
		if(!empty($_FILES[$nombreArchivo])) {
			...
			if($this->verificarArchivo($extension, $archivo['size'])) {
				...
				http_response_code(500);
				echo 'La extensión o peso del archivo no son válidos.';
				exit();
			}
			...
			http_response_code(500);
			echo  'No has proporcionado ningún archivo';
			exit();
		}
	...
	function verificarArchivo($ext, $size)
	{
		...
		if($ext=='svg' || $ext=='php' || $ext=='php1' || $ext=='php2' || $ext=='php3' || $ext=='php4' || $ext=='php5' || $ext=='php6' || $ext=='php7' || $ext=='phtml'
    ...
    function verificarArchivoImagen($ext, $tamano)
    {
        if($ext=='jpg' || $ext=='jpeg' || $ext=='png'|| $ext=='tiff' || $ext=='svg' || $tamano > 100000000 )
        {
            $aux=1;
        }
